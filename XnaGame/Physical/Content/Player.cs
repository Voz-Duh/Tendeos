...
using XnaGame.Utils.SaveSystem.Content;
using XnaGame.World;
...
namespace XnaGame.Physical.Content
{
    public class Player : SpawnEntity, ITransform
    {
        ...
        public override void Update()
        {
            ...
            {
                new Item(inventory.Items[inArm], transform.Position);
                inventory.Items[inArm] = default;
            }
        }
        ...
        public void ToByte(ByteBuffer buffer)
        {
            ...
            buffer.Append(transform.body.velocity.X);
            buffer.Append(transform.body.velocity.Y);
            buffer.Append(transform.flipX);
            buffer.Append(info);
            buffer.Append(inArm);
        ...
        {
            transform.Position = new Vec2(buffer.ReadFloat(), buffer.ReadFloat());
            transform.body.velocity = new Vec2(buffer.ReadFloat(), buffer.ReadFloat());
            transform.flipX = buffer.ReadBool();
            buffer.Read(info);
            inArm = buffer.ReadByte();
