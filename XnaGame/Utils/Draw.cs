...
namespace XnaGame.Utils
{
    public static class SDraw
    {
        ...
        public static void Text(SpriteFont font, string text, FVector2 position, float rotation = 0, float scale = 1, float depth = 0, Origin xOrigin = Origin.Center, Origin yOrigin = Origin.Center) =>
            Text(font, Color.White, text, position, rotation, scale, depth, xOrigin, yOrigin);

        public static void Text(SpriteFont font, string text, FVector2 position, FVector2? scale, float rotation = 0, float depth = 0, Origin xOrigin = Origin.Center, Origin yOrigin = Origin.Center) =>
            Text(font, Color.White, text, position, scale, rotation, depth, xOrigin, yOrigin);

        public static void Text(SpriteFont font, Color color, string text, FVector2 position, float rotation = 0, float scale = 1, float depth = 0, Origin xOrigin = Origin.Center, Origin yOrigin = Origin.Center)
        {
            FVector2 size = font.MeasureString(text);
            spriteBatch.DrawString(
                ...
                color,
                MathHelper.ToRadians(rotation),
                new FVector2(size.X * GetOrigin(xOrigin), size.Y * GetOrigin(yOrigin)),
                scale,
                SpriteEffects,
                depth
            );
        ...
        public static void Text(SpriteFont font, Color color, string text, FVector2 position, FVector2? scale, float rotation = 0, float depth = 0, Origin xOrigin = Origin.Center, Origin yOrigin = Origin.Center)
        {
            FVector2 size = font.MeasureString(text);
            spriteBatch.DrawString(
                font,
