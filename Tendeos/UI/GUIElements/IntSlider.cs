        ...
        public override void Update(FRectangle rectangle)
        {
            ...
            if (MouseOn && Mouse.LeftDown)
                ...
                unsafe
                {
                    ...
                    {
                        case Slider.Type.Up2Down:
                            res = (int)MathF.Round(Math.Clamp((Mouse.GUIPosition.Y - (rectangle.Bottom + style.Bar.Start.Value)) / (rectangle.Height - style.Bar.End.Value - style.Bar.Start.Value), 0, 1) * sections);
                            break;
                        case Slider.Type.Down2Up:
                            res = (int)MathF.Round(Math.Clamp((rectangle.Top - style.Bar.End.Value - Mouse.GUIPosition.Y) / (rectangle.Height - style.Bar.End.Value - style.Bar.Start.Value), 0, 1) * sections);
                            break;
                        case Slider.Type.Left2Right:
                            res = (int)MathF.Round(Math.Clamp((Mouse.GUIPosition.X - (rectangle.Left + style.Bar.Start.Value)) / (rectangle.Width - style.Bar.End.Value - style.Bar.Start.Value), 0, 1) * sections);
                            break;
                        case Slider.Type.Right2Left:
                            res = (int)MathF.Round(Math.Clamp((rectangle.Right - style.Bar.End.Value - Mouse.GUIPosition.X) / (rectangle.Width - style.Bar.End.Value - style.Bar.Start.Value), 0, 1) * sections);
                            break;
                    }
                    set(res);
                }
